...
import { adminCounts, adminDonations, adminInquiries, adminLogs } from '@/lib/data/admin';
import { money, dateTime } from '@/lib/format';
import { AdminTitle, AdminBadge } from '@/components/admin/AdminUI';
import { hasSupabase } from '@/lib/env';

export default async function AdminDashboard() {
  const [counts, pending, inquiries, logs] = await Promise.all([adminCounts(), adminDonations({ status: 'pending' }), adminInquiries(), adminLogs()]);
  const tiles: [string, number, string, string][] = [
    ['입금 확인 대기', counts.pendingDonations, 'donations?status=pending', 'receipt-line'], ['환불 처리 대기', counts.refunds, 'refunds', 'refund-2-line'],
    ['미답변 문의', counts.inquiries, 'inquiries', 'mail-unread-line'], ['신고 검토 대기', counts.reports, 'moderation', 'flag-line']
  ];
  return (
    <>
      <AdminTitle title="운영 대시보드" text="오늘 처리할 업무를 한눈에 확인하세요.">{!hasSupabase && <span className="badge status-pending">미리보기 모드</span>}</AdminTitle>
      ...
      <div className="admin-dashboard-grid">
        <section className="admin-dashboard-section"><h2>입금 확인 대기 <Link href="/admin/donations?status=pending">전체 보기</Link></h2>
