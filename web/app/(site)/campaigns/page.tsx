...
export default async function CampaignsPage({ searchParams }: { searchParams: Promise<{ view?: string; category?: string; q?: string }> }) {
  ...
  const category = ['matching', 'cheer'].includes(sp.category ?? '') ? sp.category! : '';
  const q = (sp.q ?? '').trim();
  const now = Date.now();
    ...
    <>
      ...
      <div className="container program-page">
        ...
        <div className="program-toolbar">
          <Chips items={[['', '전체', url({ category: '' })], ['matching', '기업 매칭', url({ category: 'matching' })], ['cheer', '응원 참여', url({ category: 'cheer' })]]} current={category} label="캠페인 유형" />
        ...
        {items.length ? <div className="program-grid">{items.map((c) => <ImageCard key={c.id} image={c.image ?? '/assets/images/community.jpg'} category={c.type === 'matching' ? '기업 매칭' : '응원 참여'} title={c.title} body={c.description} href={`/campaigns/${c.slug}`} cta={view === 'ended' ? '캠페인 살펴보기' : '함께 참여하기'} />)}</div> : <Empty title="해당하는 캠페인이 없습니다" text="검색어 또는 캠페인 유형을 변경해보세요." cta={false} />}
      </div>
    </>
  );
}
