  ...
  if (view === 'notices') {
    ...
    body = (
      <>
        ...
        <div className="community-board"><table><caption className="qm-sr-only">퀸만덕 공지사항</caption><thead><tr><th scope="col">번호</th><th scope="col">분류</th><th scope="col">제목</th><th scope="col">등록일</th></tr></thead>
          <tbody>{items.map((c, i) => <tr key={c.id}><td className="community-board-number">{filtered.length - (page - 1) * per - i}</td><td className="community-board-category">공지사항</td><td className="community-board-title"><Link href={`/support/notices/${c.slug ?? c.id}`}>{c.title}</Link></td><td className="community-board-date">{date(c.created_at)}</td></tr>)}</tbody></table>{!filtered.length && <EmptyResult />}</div>
