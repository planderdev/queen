...
import { adminRegistrations } from '@/lib/data/admin';
...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rows = await adminRegistrations(id);
  const header = ['신청일시', '성함', '연락처', '이메일', '성별', '연령대', '참가비 입금자명', '회원 여부', '상태', '메모'];
  const lines = rows.map((r) => [dateTime(r.created_at), r.name, r.phone ?? '', r.email, r.gender ?? '', r.age_group ?? '', r.depositor_name ?? '', r.user_id ? '회원' : '비회원', statusNames[r.status] ?? r.status, r.note ?? ''].map(cell).join(','));
