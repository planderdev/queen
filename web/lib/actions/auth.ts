...
// Supabase Auth 오류를 사용자 문구로 번역
function authMessage(message: string) {
  const m = message.toLowerCase();
  if (m.includes('invalid login credentials')) return '이메일 또는 비밀번호가 맞지 않습니다.';
  if (m.includes('email not confirmed')) return '이메일 인증을 먼저 완료해주세요. 받은편지함을 확인해주세요.';
  if (m.includes('already registered') || m.includes('already been registered')) return '이미 가입된 이메일입니다. 로그인해주세요.';
  if (m.includes('is invalid') && m.includes('email')) return '사용할 수 없는 이메일 주소입니다. 실제 사용하는 이메일을 입력해주세요.';
  if (m.includes('rate limit')) return '잠시 후 다시 시도해주세요. 인증 메일 발송 한도에 도달했습니다.';
  if (m.includes('password') && m.includes('at least')) return '비밀번호는 8자 이상으로 입력해주세요.';
  return message;
}
