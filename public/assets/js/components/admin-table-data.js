...
export function recordAt(index,tableIndex=0){
 const q=new URLSearchParams(location.search),s=read();
 const keys={users:['users'],organizations:['organizations'],fundraisers:['fundraisers'],transactions:['donations'],recurring:['recurringPlans'],refunds:['refundRequests'],payouts:['payouts'],reports:['impactReports'],campaigns:['campaigns','matchingContributions'],content:['content'],moderation:['moderationReports'],inquiries:['inquiries'],logs:['auditLogs']};
 let rows=s[(keys[q.get('view')]||[])[tableIndex]]||[];
