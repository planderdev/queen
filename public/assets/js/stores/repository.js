...
export function read(){
 ...
 try {const raw=globalThis.localStorage?.getItem(STORAGE_KEY); if(raw){const parsed=JSON.parse(raw);if(parsed.schema!==config.schema){memory=makeSeed();if(parsed.schema<config.schema){globalThis.localStorage?.setItem(STORAGE_KEY,JSON.stringify(memory));storageWarning='데모 데이터 구조가 바뀌어 새 데모로 시작했습니다.';}else storageWarning='다른 탭이 더 새로운 데모 데이터를 사용하고 있어 이 탭은 임시 데이터로 표시됩니다. 새로고침해주세요.';}else{if(tables.some(k=>!Array.isArray(parsed[k]))||!parsed.drafts||!parsed.settings||!Number.isFinite(Date.parse(parsed.clock))||!parsed.users.length)throw Error('schema'); memory=parsed;}} else {memory=makeSeed();globalThis.localStorage?.setItem(STORAGE_KEY,JSON.stringify(memory));}}
...
export function write(data){
 ...
 try{const raw=globalThis.localStorage?.getItem(STORAGE_KEY);const stored=raw?JSON.parse(raw).schema:0;if(stored>config.schema)throw Error('newer');globalThis.localStorage?.setItem(STORAGE_KEY,JSON.stringify(next));}catch{storageWarning='브라우저 저장 공간을 사용할 수 없어 이 탭에서만 변경사항이 유지됩니다.';}
